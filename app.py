...
from utils.helpers import trigger_github_action
...
def main():
    city_id, station_name, time_slot, email, subscribe_button = render_ui(CITIES)
    ...
    if subscribe_button:
        try:
            ...
            add_or_update_preference(email, station_name, time_slot.strftime('%H:%M'))

            if trigger_github_action(email, station_name, time_slot):
                st.success(
